	...
	Lesson2--算法的 时间复杂度 和 空间复杂度
		...
		3. 空间复杂度
			...
			实例
				...
				实例1：计算BubbleSort的空间复杂度？-冒泡排序
				实例2：计算Fibonacci的空间复杂度？-斐波那契数列
	...
	*/
	/*
		空间复杂度
			...
			Fibonacci(N - 1) + ________________
				[空间1]
				↓     ↑×
				[空间2]
				↓     ↑×
			...
			________________ + Fibonacci(N - 2)
				[空间1]
				↓     ↑×
				[空间2]
				↓     ↑×
...
void BubbleSort(int* a, int n)
{
	...
	/*
		...
		空间复杂度也是一个数学函数表达式，是对一个算法在运行过程中临时 额外 占用存储空间大小的量度。

		(int* a, int n)这些是本身需要的

		额外的是算法中定义的变量 size_t end、size_t i

		内层循环结束后 i 销毁

		再次循环进入 内层循环 再定义一个 i ，和之前的 i 共用同一块空间
...
long long* Fibonacci(size_t n)
{
	...
	/*
		...
		空间复杂度
			...
			给定0个和第一个：fibArray[0] = 0; fibArray[1] = 1;

			第零个和第一个可以算出第二个：fibArray[i] = fibArray[i - 1] + fibArray[i - 2];
			第一个和第二个可以算出第三个：fibArray[i] = fibArray[i - 1] + fibArray[i - 2];
			......
