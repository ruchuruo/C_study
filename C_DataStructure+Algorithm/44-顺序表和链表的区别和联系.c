/*
	Lesson3--顺序表和链表
		...
		4. 顺序表和链表的区别和联系
			...
			顺序表：一白遮百丑
				优点
					1. 支持随机访问(下标访问)，需要 随机访问结构 支持的 算法 可以很好的适用。
					2. CPU高度缓存 命中率 更高。

				缺点
					1. 插入删除时间效率低，时间复杂度O(N)。
					2. 连续的物理空间，空间不够需要增容。
					   增容有一定程度的消耗。为了避免频繁增容，一般都按倍数增容，用不完存在浪费。
			...
			链表(双向带头循环)：一黑毁所有
				...
				缺点
					1. 不支持随机访问(下标访问)，一些 排序、二分查找 等算法在这种结构上不适用。
					2. 链表存储一个值，同时要存储链表指针，有一定的消耗。
					3. CPU高度缓存 命中率 更低。
...
|                                            ↗
| 更小                                 L0: ↗ 寄存器                   -
| 更快                                   ↗--------------------------- | CPU寄存器 保存着从 高速缓存存储器 取出的字
| (每字节)                         L1: ↗   高速缓存(SRAM)             -
| 成本更高的存储设备                  ↗------------------------------- | L1高速缓存 保存着从 L2高速缓存 取出的缓存行
|                              L2: ↗       高速缓存(SRAM)             -
                                 ↗----------------------------------- | L2高速缓存 保存着从 L3高速缓存 取出的缓存行
                           L3: ↗           高速缓存(SRAM)             -
|                            ↗--------------------------------------- | L3高速缓存 保存着从 主存高速缓存 取出的缓存行
| 成本更低的存储设备        ↗                 主存(DRAM)               -
| (每字节)               ↗------------------------------------------- | 主存 保存着从 本地磁盘 取出的磁盘块
| 更慢                 ↗                本地二级存储(本地磁盘)         -
| 更大               ↗----------------------------------------------- | 本地磁盘 保存着从 远程网络服务器磁盘 取出的文件
...
↓                 ----------------------------------------------------
			...
			分别遍历 顺序表 和 链表
				...
				访问存储1数据的内存位置0x00123400，地址在 高速缓存 中就直接访问。
				不在(没命中)就先加载到 高速缓存，再访问。

				加载会加载一定范围的内存，
				顺序表是连续的物理空间，加载一次就能访问到一部分 有用数据。
				链表不是连续的物理空间，加载一次可能就只访问到一个 有用数据 ，其他 无用的数据 占用缓存，造成 缓存污染。


			与程序员相关的CPU缓存知识
			https://www.coolshell.cn/articles/20793.html
