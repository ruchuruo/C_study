			...
			○ 枚举的优点
				...
				我们可以使用#define 定义常量，为什么非要使用枚举？
					...
					4. 便于调试
						...
						#define 定义的符号，比如：		#define red 5
						使用：							int a = red;
						red被替换成5，是在预编译阶段：	int a = red;
						预编译结束后：					int a = 5;
						转换成二进制
						在调试的时候就看不见red这样的符号了

						枚举不是替换，运行期间是二进制，这个二进制对应的就是枚举写的代码
...
void 枚举类型的定义()
{
	...
	enum 颜色
	{
		红 = 5,// 即使是常量，也应该有值，这里是赋初始值。常量不能修改，这里是赋初始值
		绿,
		蓝,
	};
	...
	// 通过这个类型创建变量
	enum 星期 a = 星期一;// 变量a，取值是其中的一种
	//enum 星期 a = 2;// 虽然可以这样取值但是不建议，C++中报错
	//enum 星期 a = 星期::星期一;// :: 限定符，限定 星期一 来自 星期
...
void 联合的特点()
{
	...
	// 打印地址
	printf("%p\n", &un);										// 000000FDB86FF674
	printf("%p\n", &(un.c));// un.c是一个变量，要括起来再取地址	// 000000FDB86FF674
	printf("%p\n", &(un.i));									// 000000FDB86FF674
...
void 联合的特点_初始化()
{
	...
	/*
		...
		多个成员如果想共用同一块空间可以使用联合体
		比如
			设计一个小程序来判断当前机器的字节序
			D:\ruchuruo\C\Math\math\判断当前机器的字节序(联合体).c
	*/
...
void 联合大小的计算()
{
	...
	union Un
	{
		char a[5];	// 占5个字节，char类型 对齐数按1来算，char a数组5个元素 和 放5个char类型变量一样
		int i;		// 占4个字节，int 类型 对齐数按4来算
	};
