/*
	动态内存管理
		...
		● 柔性数组
			...
			柔性数组(flexible array)，C99中，结构中的最后一个元素允许是未知大小的数组，
			这就叫做 柔性数组 成员。
			...
			柔性数组的好处
				...
				方便内存的释放
					...
					并返回给用户一个结构体 指针，用户做一次free就可以把所有的内存也给释放掉。

				提高访问速度
					连续的内存有益于提高访问速度，也有益于减少内存碎片。
					(其实，个人觉得也没多高了，反正你跑不了要用做偏移量的加法来寻址)
...
/*
	...
	局部性原理
		...
		空间局部性（spatial locality）
			一旦程序访问了某个存储单元，在不久之后，其附近的存储单元也将被访问，即程序在一段时间内所访问的地址，
...
void 柔性数组()
{
	...
	// 正确的创建方法
	// 期望arr的大小是10个整形
	// sizeof(struct S2)计算柔性数组前成员的大小
	struct S2* ps2 = (struct S2*)malloc(sizeof(struct S2) + 10 * sizeof(int));
	...
	// 增加
	struct S2* ptr = (struct S2*)realloc(ps2, sizeof(struct S2) + 20 * sizeof(int));
	if (ptr != NULL)
	{
		ps2 = ptr;
	}
...
void 柔性数组II()
{
	...
	// 释放
	// 先释放第二块空间，因为第一块主空间释放后就找不到第二块空间了
	free(ps->arr);
	ps->arr = NULL;
	free(ps);
	ps = NULL;
	...
	/*
		...
		能达到柔性数组的效果，但是 不是柔性数组，
		两次malloc就要两次free，容易出错。
		柔性数组只用malloc一次。

		维护堆空间的时候，多次开辟空间，不同的空间之间会有间隙(内存碎片)，
		再次利用的可能性低。
	*/
